import { createIncrementHandler, createStatsHandler, HandlerEvent } from '../handlers';
...
describe('createIncrementHandler', () => {
  it('rejects non-POST requests with 405', async () => {
    ...
    const result = await handler(fakeEvent('GET'));
  ...
  it('increments the store and returns the resulting totals as JSON on POST', async () => {
    ...
    const result = await handler(fakeEvent('POST'));
...
describe('createStatsHandler', () => {
  ...
  it('returns the store totals as JSON on GET', async () => {
    ...
    expect(result.statusCode).toBe(200);
    expect(JSON.parse(result.body)).toEqual({ total_today: 42, total_all_time: 1204996 });
  });
